...
class CustomerPartialPayment(models.Model):
    _name = "customer.partial.payment"
    _order = 'name desc'

    name = fields.Char("Name", default="Draft Receipt")
    partner_id = fields.Many2one('res.partner', string="Partner", domain=[('customer', '=', True)], required=True)
    ...
    debit_line_ids = fields.One2many('customer.partial.payment.line', 'multi_pay_id', string="Debit Line", domain=[('payment_id.payment_type', '=', 'inbound')])
    credit_line_ids = fields.One2many('customer.partial.payment.line', 'multi_pay_id', string="Credit Line", domain=[('invoice_id.type', '=', 'out_invoice')])
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('customer.partial.payment')
        return super(CustomerPartialPayment, self).create(vals) 
    ...
    def create_payments(self):
        ...
        if (self.paid_amount + abs(debit)) != credit:
                ...
                for lines in self.debit_line_ids:
                    if lines.payment_id:
                        ...
                        for line_id in move_line_payment_id:
                                if line_id.customer_partial_pay_id != lines.id:
                                    line_id.customer_partial_pay_id = lines.id
                    self.trans_rec_reconcile_payment(move_line_payment_id,move_line_invoice_id)
...
class CustomerPartialPaymentLine(models.Model):
    _name = "customer.partial.payment.line"

    description = fields.Char('Customer Invoice')
    ...
    multi_pay_id = fields.Many2one("customer.partial.payment", "Payment ID")
    ...
    def create(self, vals):
        ...
        return super(CustomerPartialPaymentLine, self).create(vals)
    ...
    def write(self, vals):
        ...
        if vals.get('amount_residual'):
            vals['amount_residual'] = vals.get('amount_residual')
        return super(CustomerPartialPaymentLine, self).write(vals) 


class AccountMoveLine(models.Model):
    _inherit = "account.move.line"

    customer_partial_pay_id = fields.Many2one("customer.partial.payment.line")
    ...
    def auto_reconcile_lines(self, invoice_type=False, invoice=None):
        ...
        if sm_credit_move:
            if not sm_credit_move.invoice_id:
                if sm_credit_move.customer_partial_pay_id:
                    amount_to_reconcile += sm_credit_move.customer_partial_pay_id.amount_to_pay
                    amount_reconcile = amount_to_reconcile
                    sm_credit_move.customer_partial_pay_id = False
