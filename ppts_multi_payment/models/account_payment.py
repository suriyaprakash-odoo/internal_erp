...
class AccountMove(models.Model):
    ...
    cus_multi_pay_id = fields.Many2one("customer.multi.payment", "Payment ID", copy=False)
    ven_multi_pay_id = fields.Many2one("vendor.multi.payment", "Payment ID", copy=False)
    
    # Set partner_id domain for customer
    @api.onchange('partner_type')
    def _onchange_partner_type(self):
        if self.partner_type:
            if self.partner_type == 'customer':
                return {'domain': {'partner_id': [(self.partner_type, '=', True),('billing','=',True)]}}
            else:
